#!/usr/bin/env python3
"""
Evaluate Terminus on the APEX benchmark.

Usage:
    python run_apex_terminus.py [--model MODEL] [--limit N] [--domains DOMAIN1,DOMAIN2] [--grading-model MODEL]
...
Example:
    ...
    python run_apex_terminus.py
    
    # Run on 10 samples from Legal domain
    python run_apex_terminus.py --model gpt-4o --limit 10 --domains Legal
    
    # Run with specific grading model
    python run_apex_terminus.py --model claude-sonnet-4-20250514 --grading-model gpt-4o
"""
...
def setup_logging(verbose: bool = False) -> logging.Logger:
    ...
    return logging.getLogger("apex_terminus_eval")


def parse_args() -> argparse.Namespace:
    """Parse command line arguments."""
    parser = argparse.ArgumentParser(
        description="Evaluate Terminus on APEX benchmark",
    ...
    parser.add_argument(
        "--session-name",
        type=str,
        default="terminus-apex-eval",
        help="Tmux session name for Terminus (default: terminus-apex-eval)",
    )
    ...
    parser.add_argument(
        ...
        help="Comma-separated list of domains to evaluate (default: all). "
             "Options: Finance, Consulting, Legal, Medicine",
    )
    parser.add_argument(
        "--shuffle",
        action="store_true",
...
def main():
    ...
    # Initialize Terminus orchestrator
    logger.info(f"Initializing Terminus with model: {args.model}")
    orchestrator = Terminus(model=args.model)
    ...
    # Run the benchmark
    logger.info("=" * 60)
    logger.info("Starting APEX Evaluation with Terminus")
    logger.info("=" * 60)
    ...
    try:
        ...
        results_file = output_dir / f"apex_terminus_{model_name}_{timestamp}.json"
        
        output_data = {
            "orchestrator": "terminus",
            "config": {
                "model": args.model,
