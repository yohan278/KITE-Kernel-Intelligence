#!/usr/bin/env python3
"""Run RL kernel training experiment (GRPO) matching exact log format.

Usage:
    python scripts/train_rl.py --config configs/exp/<experiment>/seed{seed}.yaml

Referenced by M1 (throughput_rl), M2 (energy_aware_rl), and M3 (ipw_blend) logs.
Detects stage from config: kernel_grpo_throughput / kernel_grpo_energy / kernel_grpo_ipw.
"""
...
class ExperimentLogger:
    ...
    def write_preamble(self, host, gpu_count, stage, workers, prefetch, command_cfg):
        ...
        self.info(f'command="python scripts/training/train_rl.py --config configs/exp/{command_cfg}/seed{{seed}}.yaml"')
...
def detect_stage(cfg: dict) -> str:
    grpo = cfg.get("grpo", {})
    if grpo.get("reward_ipw_blend_weight", 0) > 0 or "ipw" in str(cfg.get("experiment", "")):
        return "kernel_grpo_ipw"
    if grpo.get("energy_aware", False) or grpo.get("reward_beta_joules", 0) > 0:
        return "kernel_grpo_energy"
    return "kernel_grpo_throughput"


def run_rl_seed(seed, args, cfg, adapter, log, exp_name_part="rl"):
    set_seed(seed)
    seed_dir = (args.output or PROJECT_ROOT / "checkpoints" / "rl") / f"seed{seed}"
    ...
    grpo_config = GRPOKernelConfig(output_dir=seed_dir)
...
def parse_args():
    p = argparse.ArgumentParser(description="RL kernel training experiment runner")
...
def main():
    ...
    model_tag = {"kernel_grpo_throughput": "M1_GRPO_THROUGHPUT",
                 "kernel_grpo_energy": "M2_GRPO_ENERGY",
                 "kernel_grpo_ipw": "M3_GRPO_IPW_BLEND"}.get(stage, "M1_GRPO_THROUGHPUT")
    ...
    seed_results = []
    for seed in SEEDS:
        metrics = run_rl_seed(seed, args, cfg, adapter, log, exp_name_part)
    ...
    log.close()
    logger.info("RL experiment %s (%s) complete", exp_full, stage)
