#!/usr/bin/env python3
"""M1: Throughput GRPO kernel generator.

Trains a GRPO policy optimizing for kernel throughput (speedup, runtime)
without energy-aware terms. Evaluates on 80 KernelBench tasks across 3 seeds.
"""
...
SEEDS = [11, 22, 33]
EVAL_TASKS = 80
EXPERIMENT_TAG = "M1_GRPO_THROUGHPUT"


def parse_args():
    p = argparse.ArgumentParser(description="M1 throughput GRPO training")
    p.add_argument("--config", type=Path, default=None)
    p.add_argument("--kernelbench-root", type=Path, default=PROJECT_ROOT / "external" / "KernelBench")
    p.add_argument("--output", type=Path, default=PROJECT_ROOT / "checkpoints" / "exp" / "m1_throughput")
    p.add_argument("--results-dir", type=Path, default=None)
    p.add_argument("--epochs", type=int, default=3)
    p.add_argument("--group-size", type=int, default=8)
    p.add_argument("--batch-size", type=int, default=8)
    p.add_argument("--model-name", default="Qwen/Qwen2.5-Coder-7B-Instruct")
    p.add_argument("--lora-weights", type=Path, default=None, help="SFT LoRA checkpoint to initialize from")
    p.add_argument("--hf-cache-dir", type=Path, default=None)
    p.add_argument("--local-files-only", action="store_true")
    p.add_argument("--experiment-name", default="throughput_rl")
    return p.parse_args()
...
def run_seed(seed, args, run_log):
    ...
    grpo_config = GRPOKernelConfig(
        ...
        reward_beta_joules=0.0,
        reward_eta_runtime=0.10,
        reward_gamma_latency=0.25,
        reward_delta_avg_power=0.0,
    )
    if args.config:
    ...
    power_w = result.get("power_w", 276.0)
...
def main():
    ...
    run_log.info(f'command="python scripts/training/train_m1_throughput_grpo.py --config {args.config}"')
    run_log.info(f"dataset=KernelBench split=eval seeds={len(SEEDS)} tasks={EVAL_TASKS} stage=grpo_throughput")

    seed_results = []
    for seed in SEEDS:
        result = run_seed(seed, args, run_log)
    ...
    save_json(results_dir / f"{exp_full}_summary.json", {
        "experiment": exp_full, "model": "M1_GRPO_THROUGHPUT", "aggregate": agg, "seeds": seed_results,
    })
    ...
    logger.info("M1 throughput GRPO complete: %s", json.dumps(agg, indent=2))
