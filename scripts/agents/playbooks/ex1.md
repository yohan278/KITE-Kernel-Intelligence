...
## Objective
Claim queued runs and generate runnable GPU job scripts for worker slot 1.
...
```bash
bash scripts/agents/wrappers/ex1.sh
```
...
4. Write done marker `outputs/agent_queue/state/ex1.done`.
## Expected Artifacts

- outputs/agent_queue/jobs/*.sh
- outputs/agent_queue/state/ex1.done
...
```text
[ex1] complete: prepared job scripts and updated queue statuses to prepared.
```
