...
## Objective
Claim queued runs and generate runnable GPU job scripts for worker slot 0.
...
4. Write done marker `outputs/agent_queue/state/ex0.done`.
## Expected Artifacts

- outputs/agent_queue/jobs/*.sh
- outputs/agent_queue/state/ex0.done
...
```text
[ex0] complete: prepared job scripts and updated queue statuses to prepared.
```
