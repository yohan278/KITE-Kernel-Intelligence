...
## Objective
Generate inference-budget ablation configs.
...
```bash
bash scripts/agents/wrappers/sg6.sh
```

3. Verify the done marker:

```bash
ls outputs/agent_queue/state/sg6.done
```

## Expected Artifacts

- configs/exp/abl_budget/*.yaml

## Validation Checklist

1. No YAML syntax errors.
2. Files are under `configs/exp/...` only.
3. `outputs/agent_queue/state/sg6.done` exists.

## Handoff Message

```text
[sg6] complete: generated configs, wrote done marker at outputs/agent_queue/state/sg6.done
```
