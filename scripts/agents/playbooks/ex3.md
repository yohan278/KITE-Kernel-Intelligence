...
## Objective
Claim queued runs and generate runnable GPU job scripts for worker slot 3.
...
```bash
bash scripts/agents/wrappers/ex3.sh
```
...
4. Write done marker `outputs/agent_queue/state/ex3.done`.
## Expected Artifacts

- outputs/agent_queue/jobs/*.sh
- outputs/agent_queue/state/ex3.done
...
```text
[ex3] complete: prepared job scripts and updated queue statuses to prepared.
```
