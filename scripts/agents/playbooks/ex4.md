...
## Objective
Claim queued runs and generate runnable GPU job scripts for worker slot 4.
...
```bash
bash scripts/agents/wrappers/ex4.sh
```
...
4. Write done marker `outputs/agent_queue/state/ex4.done`.
## Expected Artifacts

- outputs/agent_queue/jobs/*.sh
- outputs/agent_queue/state/ex4.done
...
```text
[ex4] complete: prepared job scripts and updated queue statuses to prepared.
```
