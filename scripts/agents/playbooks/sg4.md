...
## Objective
Generate reward-term ablation configs.
...
```bash
bash scripts/agents/wrappers/sg4.sh
```

3. Verify the done marker:

```bash
ls outputs/agent_queue/state/sg4.done
```

## Expected Artifacts

- configs/exp/abl_reward/*.yaml

## Validation Checklist

1. No YAML syntax errors.
2. Files are under `configs/exp/...` only.
3. `outputs/agent_queue/state/sg4.done` exists.

## Handoff Message

```text
[sg4] complete: generated configs, wrote done marker at outputs/agent_queue/state/sg4.done
```
