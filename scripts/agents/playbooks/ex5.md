...
## Objective
Claim queued runs and generate runnable GPU job scripts for worker slot 5.
...
```bash
bash scripts/agents/wrappers/ex5.sh
```
...
4. Write done marker `outputs/agent_queue/state/ex5.done`.
## Expected Artifacts

- outputs/agent_queue/jobs/*.sh
- outputs/agent_queue/state/ex5.done
...
```text
[ex5] complete: prepared job scripts and updated queue statuses to prepared.
```
